...
public class ImageDetailFragment extends Fragment implements LifecycleRegistryOwner, Injectable{
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        mImageDetailViewModel.isNetworkOnline().removeObservers(this);
        ...
        mImageDetailViewModel.isLoading().observe(this, mBinding.get().imageSrl::setRefreshing);
        mImageDetailViewModel.getImagesListLiveData().observe(this, imageResponse -> {
            if (imageResponse == null)
                return;
            if (imageResponse.status == Status.ERROR && imageResponse.throwable != null)
                Snackbar.make(
                        mBinding.get().getRoot(),
                        imageResponse.throwable.getLocalizedMessage(),
                        Snackbar.LENGTH_LONG

                ).show();
            else
                mBinding.get().setImage(imageResponse.data);
        });
        mImageDetailViewModel.loadImage(getArguments().getString(IMAGE_ID_KEY));
    }
